...
router.post('/create', async (req, res) => {
    ...
    try{
        ...
        return res.status(500).send({ error: "Erro ao criar o usuário!" + err })
    }
})
...
router.post('/auth' , async (req, res) => {
    const { email, password } = req.body
    if (!email || !password) return res.status(400).send({ error:'Error dados insuficientes' })
    ...
    try{
        ...
        if(!pass_ok) return res.status(401).send({error: "Senha invalida"})
